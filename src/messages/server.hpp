...
public:
    explicit GameServerMessage(Player &player) : GameMessage(player), m_unique(true), m_abort(false) { }
    ...
    const GameServerMessage_t& getType() const { return m_type; }
    ...
    virtual std::shared_ptr<GameServerMessage> getServerPacket() override { return shared_from_this(); }
